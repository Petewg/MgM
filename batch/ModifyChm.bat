...
:SYNTAX
  ...
  echo free download at http://www.microsoft.com/downloads/details.aspx?FamilyID=00535334-c8a6-452f-9aa0-d597d16580cc&displaylang=en.
  echo.
  echo Syntax:
  echo ModifyChm (source) [?] [/D] [/C] [/G] [/DL (loc)] [/CX]
  echo           [/GD] [/GT (title)] [/GI (init)] [/P]
  echo.
  ...
  echo   /GT         For /G, use alternate title to display in CHM
  echo   (title)     Alternate CHM title, default (source)
  echo   /GI         For /G, use alternate initial file to display in CHM
  echo   (init)      Alternate CHM initial file, including extension,
  echo                 default (source).htm
  echo   /P          Pause at end
  echo.
...
:GENHHP
  ...
  echo base="%MV_GTITLE%","%MV_HHC%","%MV_HHK%","%MV_GINIT%","%MV_GINIT%",,,,,0x1d20,250,0x304e,[10,10,700,500],0x0,0x0,1,0,0,0, >>%MV_SRC%.hhp
  goto GSUCCESS
