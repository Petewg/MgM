...
DEFINE WINDOW Form_1 ;
  ...
  DEFINE MAIN MENU
    ...
    END POPUP
    POPUP "Help"
      ITEM "&About" ACTION MsgInfo( "MiniGUI MySQL Access Sample by"+CRLF+"Mitja Podgornik <yamamoto@rocketmail.com>" )
    END POPUP
  END MENU
...
Else
  ...
  Endif
  oRow	:= oQuery:GetRow(1)
  cCode	:= Alltrim(Str(oRow:fieldGet(1) + 1))
  If Empty(Val(cCode) - 1)
     oQuery:Destroy()
     oQuery:= oServer:Query( "Select Code From NAMES" )
     oRow  := oQuery:GetRow(oQuery:LastRec())
     cCode := Alltrim(Str(oRow:fieldGet(1) + 1))
  Endif
  oQuery:Destroy()
EndIf         
