...
Local fColor := { |val, CellRowIndex| if ( CellRowIndex/2 == int(CellRowIndex/2) , RGB( 0,0,255 ) , RGB( 0,255,0 ) ) }	
	...
	DEFINE WINDOW Form_1 ;
		...
		@ 10,10 GRID Grid_1 ;
			...
			ITEMS aRows ;
			EDIT ;
			INPLACE { {'TEXTBOX','NUMERIC','$ 999,999.99'} , {'DATEPICKER','DROPDOWN'} , {'COMBOBOX',{'One','Two','Three'}} , { 'SPINNER' , 1 , 20 } , { 'CHECKBOX' , 'Yes' , 'No' } } ;
...
LOCAL a
	...
	aEval( a, {|x,i| msginfo ( if( i = 2, dtoc( x ), ;
		if( i = 5, (if ( x == .t. , '.t.' , '.f.' )), ;
		str ( x ) ) ), ltrim( str ( i ) ) )} )
