...
RETURN MessageBoxEx( GetActiveWindow(), cText, cCaption, nType, 0x00 )
...
Procedure ChangeModalItems
   ...
   if !empty(hwnd)
      SetWindowText( hwnd, "Modal äéáëïãïò..." )
...
*-----------------------------------------------------------------------------*
#include "hbdll32.ch"
DECLARE FindWindowA( lpClassName, lpWindowName ) IN USER32.DLL ALIAS FindWindow
*-----------------------------------------------------------------------------*
DECLARE MessageBoxExA( hWnd, lpText, lpCaption, uType, wLanguageId ) IN USER32.DLL ALIAS MessageBoxEx

//DECLARE MessageBeep( uType ) IN USER32.DLL
