/*----------------------------------------------------------------------------
 ...
 Parts of this project are based upon:
	...
	"Harbour Project"
	Copyright 1999-2014, http://harbour-project.org/
...
Return lFlag
*------------------------------------------------------------------------------*
Init Procedure _Init_HMG_PRINTER
*------------------------------------------------------------------------------*

	InstallEventHandler( '_HMG_PRINTER_SpltChldMouseClick' )
...
case cLang == "EL"   // Hellenic (a.k.a Greek)
/////////////////////////////////////////////////////////////
// HELLENIC
////////////////////////////////////////////////////////////

	_hmg_printer_usermessages [01] := 'Óåëßäá'
	_hmg_printer_usermessages [02] := 'Ðñïåðéóêüðçóç'
	_hmg_printer_usermessages [03] := '1ç óåëßäá [HOME]'
	_hmg_printer_usermessages [04] := 'Ðñïçãïýìåíç óåëßäá [PGUP]'
	_hmg_printer_usermessages [05] := 'Åðüìåíç óåëßäá [PGDN]'
	_hmg_printer_usermessages [06] := 'Ôåëåõôáßá óåëßäá [END]'
	_hmg_printer_usermessages [07] := 'ÐÞãáéíå óôç óåëßäá...'
	_hmg_printer_usermessages [08] := 'ÌåãÝèõíóç'
	_hmg_printer_usermessages [09] := 'Åêôýðùóç'
	_hmg_printer_usermessages [10] := 'Áñéè.Óåëßäáò'
	_hmg_printer_usermessages [11] := 'ÅíôÜîåé'
	_hmg_printer_usermessages [12] := 'Áêõñïí'
	_hmg_printer_usermessages [13] := 'ÅðéëïãÞ åêôõðùôÞ'
	_hmg_printer_usermessages [14] := 'ÓõññáöÞ ÁíôéãñÜöùí'
	_hmg_printer_usermessages [15] := '¸êôáóç åêôýðùóçò'
	_hmg_printer_usermessages [16] := '¼ëá'
	_hmg_printer_usermessages [17] := 'Óåëßäåò'
	_hmg_printer_usermessages [18] := 'Áðü'
	_hmg_printer_usermessages [19] := 'þò'
	_hmg_printer_usermessages [20] := 'Áíôßãñáöá'
	_hmg_printer_usermessages [21] := '¼ëåò ïé óåëßäåò'
	_hmg_printer_usermessages [22] := 'ÌïíÝò óåëßäåò'
	_hmg_printer_usermessages [23] := 'ÆõãÝò óåëßäåò'
	_hmg_printer_usermessages [24] := 'Íáß'
	_hmg_printer_usermessages [25] := '¼÷é'
	_hmg_printer_usermessages [26] := 'Êëåßóéìï'
	_hmg_printer_usermessages [27] := 'ÁðïèÞêåõóç'
	_hmg_printer_usermessages [28] := 'Ìéêñïãñáößåò'
	_hmg_printer_usermessages [29] := 'Äçìéïõñãßá ìéêñïãñáöéþí... ÐåñéìÝíåôå...'
...
*------------------------------------------------------------------------------*
Return( iif( valtype ( cName ) = 'U', 'Harbour MiniGUI Print System', cName ) )
...
HB_FUNC( _HMG_PRINTER_C_RECTANGLE )
{
   ...
   if( hdcPrint != 0 )
   {
      ...
      if( hb_parl( 12 ) && hb_parl( 13 ) )
         ...
         Rectangle( ( HDC ) hdcPrint, rect.left, rect.top, rect.right, rect.bottom );
...
HB_FUNC( INITEMFFILE )
{
   ...
   hWnd = CreateWindowEx( 0, "static", NULL, Style, hb_parni( 3 ), hb_parni( 4 ), 0, 0, hWndParent, ( HMENU ) hb_parni( 2 ), GetModuleHandle( NULL ), NULL );
