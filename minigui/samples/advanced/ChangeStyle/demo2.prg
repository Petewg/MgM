...
Function MAIN()
Local i, aRows [20] [3], bColor1, bColor2, ;
	bfrColor := {|| RGB(0,0,0)}, bbkColor := {|| RGB(255,255,255)}

	bColor1 := {|x| iif( recno()/2 == int(recno()/2), RGB(255,255,255), RGB(128,255,255) )}
	bColor2 := {|x| iif( recno()/2 == int(recno()/2), RGB(0,0,255), RGB(0,0,128) )}
	...
	DbCreate("test", {{"f1","C",10,0},{"f2","C",10,0},{"f3","C",10,0}})
	if Select("test") == 0
		Use Test
	endif
	for i:=1 to 20
		dbappend()
		Aeval(aRows [i], {|x, y| FieldPut( y, x )})
	next
	dbgotop()
	...
	DEFINE WINDOW Form_1 AT 97,62 WIDTH 597 HEIGHT 337 MAIN TITLE "Extend Disable/Enable control DEMO 2"
		...
		DEFINE MAIN MENU
			...
			POPUP "DISABLE"
				ITEM "Disable BtnTextBox_1" ACTION ( Form_1.TextBox_1.BackColor := GRAY, _ExtDisableControl ( 'TextBox_1' ,'Form_1' ), Form_1.Setfocus )
				...
				ITEM "Disable Browse_1"     ACTION ( _ExtDisableControl ( 'Browse_1' ,'Form_1' ), Form_1.Setfocus )
			...
			POPUP "ENABLE"
				ITEM  "Enable BtnTextBox_1" ACTION ( _ExtEnableControl ( 'TextBox_1' ,'Form_1' ), Form_1.TextBox_1.BackColor := YELLOW )
				...
				ITEM  "Enable Browse_1"     ACTION _ExtEnableControl ( 'Browse_1' ,'Form_1' )
