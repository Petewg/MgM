...
Caution:
This class have not a parameter's validation.
If you pass a wrong number of parameters, or the wrong types to rmchart.dll, you can get a GPF
The charts, which are not in a test, may require an additional conversion.
...
CREATE CLASS RmChart
   ...
   METHOD AddLineSeries( a,b,c,d,e, ... )  INLINE ::CallDllStd( "RMC_ADDLINESERIES", a, b, ::ToDouble(c), d, ::ToDouble(e), ... )
   ...
   METHOD Draw2Printer( a,b,c,d,e,f,... )  INLINE ::CallDllStd( "RMC_DRAW2PRINTER", a, b, ::ToDecimal(c), ::ToDecimal(d), ::ToDecimal(e), ::ToDecimal(f), ... )
   ...
   METHOD SetSeriesData( a,b,c,d, ... )    INLINE ::CallDllStd( "RMC_SETSERIESDATA", a, b, c, ::ToDouble(d), ... )
   ...
   METHOD CallDllStd( cName, ... )         INLINE ::nError := hb_DynCall( { cName, ::nHandle, HB_DYN_CALLCONV_STDCALL }, ... )
   METHOD ToDecimal( xValue )              INLINE iif( ValType( xValue ) == "N", xValue + 1.01 - 1.01, 0.0 )
