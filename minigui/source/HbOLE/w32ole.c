...
static const HB_GC_FUNCS s_gcOLE_DocFuncs =
{
   hb_oleRelease,
   hb_gcDummyMark
};
...
HB_FUNC( __HB_OLE_INIT )
{
   if( s_pSym_TOleAuto == NULL )
   {
      ...
      {
         OleInitialize( NULL );
         hb_retptrGC( hb_gcAllocate( 1, &s_gcOLE_DocFuncs ) );
         s_bInit = TRUE;
      }
...
HB_FUNC( ANSITOWIDE )  /* ( cAnsiStr ) -> cWideStr */
{
   ...
   if( cString )
   {
      BSTR wString = hb_oleAnsiToWide( (char *) cString );
...
void hb_oleItemToVariant( VARIANT *pVariant, PHB_ITEM pItem )
{
   ...
   switch( hb_itemType( pItem ) )
   {
      ...
      case HB_IT_MEMO:
      {
         ...
         sString = (char *) hb_itemGetCPtr( pItem );
...
ItemToVariant_StringArray:
                  ...
                  for( i = 0; i < rgsabound.cElements; i++ )
                     SafeArrayPutElement( parray, ( LONG * ) &i, (char *) &( sString[ i ] ) );
            ...
            switch( pVariant->n1.n2.vt )
            {
               ...
               case VT_DISPATCH:
                 ...
                 if( s_pSym_TOleAuto )
                 {
                    ...
                    hb_itemMove( s_pOleAuto, hb_stackReturnItem() );
                 }
...
static PHB_ITEM SafeArrayToArray( SAFEARRAY * parray, UINT iDim, long * rgIndices, VARTYPE vt )
{
   ...
   else
   {
      ...
      else
      {
         if( vt == VT_I1 || vt == VT_UI1 )
         {
            ...
            sArray = (char *) hb_itemGetCPtr( pArray );
   ...
   if( HB_IS_ARRAY( pArray ) && vt != VT_VARIANT )
   {
      ...
      hb_itemMove( pArray, hb_stackReturnItem() );
...
HRESULT hb_oleVariantToItem( PHB_ITEM pItem, VARIANT *pVariant )
{
   ...
   switch( pVariant->n1.n2.vt )
   {
      ...
      case VT_DISPATCH:
         ...
         hb_itemMove( pOleAuto, hb_stackReturnItem() );
